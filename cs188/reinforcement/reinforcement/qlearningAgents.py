...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        current=self.qValues[(state,action)]
        self.qValues[(state,action)]=(1-self.alpha)*current+self.alpha*(reward+self.discount*self.computeValueFromQValues(nextState))
        # self.qValues[(state,action)]=100
