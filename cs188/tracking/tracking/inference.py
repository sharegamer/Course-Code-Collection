...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    # raiseNotDefined()
    variableList = [PAC,GHOST0,GHOST1,OBS0,OBS1]
    edgeTuplesList = [(GHOST0, OBS0), (PAC, OBS0),(PAC,OBS1),(GHOST1,OBS1)]
    ...
    net = bn.constructEmptyBayesNet(variableList,  edgeTuplesList, variableDomainsDict)
    return net
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Interleave joining and eliminating variables
        for variable in eliminationOrder:
            # Join all factors containing the current variable
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, variable)
            
            # If the joined factor has more than one unconditioned variable, eliminate the current variable
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            # 如果观测到的距离为None，返回1.0的概率
            if noisyDistance is None:
                return 1.0
            else:
                return 0.0

        # 如果鬼魂不在监狱中，但观测到的距离为None，返回0.0的概率
        if noisyDistance is None:
            return 0.0
        # 计算实际的曼哈顿距离
        trueDistance = abs(pacmanPosition[0] - ghostPosition[0]) + abs(pacmanPosition[1] - ghostPosition[1])
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            ...
            # 更新新的信念分布
            newBelief[ghostPosition] = self.beliefs[ghostPosition] * prob
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            ...
            for item in newPosDist.items():
                newBelief[item[0]]+=item[1]*self.beliefs[ghostPosition]
        self.beliefs=newBelief
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        num_position=len(self.legalPositions)
        for i in range(num):
            self.particles.append(self.legalPositions[i%num_position])
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            if particle in num_dict:
                num_dict[particle]+=1
            else:
                num_dict[particle]=1
        for position in self.legalPositions:
            if position in num_dict:  
                beliefs[position]=num_dict[position]/total
            else:
                beliefs[position]=0
        return beliefs
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # raiseNotDefined()
        # self.getObservationProb(observation,gameState.getPacmanPosition(),,self.getJailPosition())
        
        
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        
        # Create a DiscreteDistribution to store the weights for each particle
        ...
        if weights.total() == 0:
            # Reinitialize particles uniformly if all weights are zero
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        new_particles=[]
        for particle in self.particles:
            new_positions=self.getPositionDistribution(gameState, particle)
            new_particles.append(new_positions.sample())
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                pro=pro*self.getObservationProb(observation[i],gameState.getPacmanPosition(),particle[i],jailPosition)
            weights[particle]=pro
        # Check if all particles have zero weight
        if weights.total() == 0:
            # Reinitialize particles uniformly if all weights are zero
            self.initializeUniformly(gameState)
