...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        for card in player.hand:
            i+=1
            if card.power(opponent_card)>max:
                max=card.power(opponent_card)
                maxi=i
            
        best_card=player.hand[maxi]
        player.hand.pop(maxi)
