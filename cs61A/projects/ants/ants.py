...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        checkplace = self.place
        while distance < self.lower_bound:
            if checkplace:
                checkplace = checkplace.entrance
                distance += 1
            else:
                return None
        while not checkplace.bees and not checkplace.is_hive and self.upper_bound > distance:
            checkplace = checkplace.entrance
            distance += 1
        if checkplace.is_hive:
            return None
        else:
            if checkplace.bees:
                return random_bee(checkplace.bees)  # REPLACE THIS LINE
            else:
                return None
        # END Problem 3 and 4
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        nowplace=self.place.exit
        while nowplace:
            if nowplace.ant:
                if nowplace.ant.is_container and nowplace.ant.ant_contained:
                    nowplace.ant.ant_contained.double()
                if not nowplace.ant.doubled:
                    nowplace.ant.double()
            nowplace=nowplace.exit
