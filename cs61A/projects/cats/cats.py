...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def f(strings):
        string=remove_punctuation(strings)
        for x in subject:
            y=split(string)
            for z in y:
                if lower(z)==x:
...
def accuracy(typed, source):
    ...
    if i==0 and j==0:
        ...
        while m<i and m<j:
            if typed_words[m]==source_words[m]:
                n+=1
            m+=1
        return n/i*100
    # BEGIN PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for x in word_list:
        if typed_word==x:
            return typed_word
    i=0
    min=diff_function(typed_word,word_list[0],limit)+1
    minnum=0
    while i<len(word_list):
        if diff_function(typed_word,word_list[i],limit)<=limit and diff_function(typed_word,word_list[i],limit)<min:
            min=diff_function(typed_word,word_list[i],limit)
            minnum=i
        i+=1
    if min<=limit:
        return word_list[minnum]
    else:
        return typed_word
...
def minimum_mewtations(typed, source, limit):
    ...
    if feline_fixes(typed, source, limit)==1:  # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
        return 1
    if feline_fixes(typed, source, limit)==0:
        return 0
        # END
    # Recursive cases should go below here
        ...
        if i==len(typed) or i==len(source):
            ...
            if limit==0:
                ...
                add =minimum_mewtations(typed[0:i]+source[i]+typed[i:],source,limit-1)+1 
                remove = minimum_mewtations(typed[0:i]+typed[i+1:],source,limit-1)+1
                substitute = minimum_mewtations(typed[0:i]+source[i]+typed[i+1:],source,limit-1)+1
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    x=get_all_times(match)
    y=get_all_words(match)
    times=[]
    i=0
    j=0
    k=0
